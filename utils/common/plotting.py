...
def plot_immatch_loader(
    data_loader, normalize_and_scale=False, num_sample=2, axis="on", dtype="triplet"
):
    import matplotlib.pyplot as plt
    ...
    for i, batch in enumerate(data_loader):
        ...
        for j in range(num):
            ...
            im_src = (
                undo_normalize_scale(im_src)
                if normalize_and_scale
                else im_src.astype(np.uint8)
            )
            im_pos = (
                undo_normalize_scale(im_pos)
                if normalize_and_scale
                else im_pos.astype(np.uint8)
            )
            axs[0].imshow(im_src)
            axs[0].axis(axis)
            axs[1].imshow(im_pos)
            ...
            if ncols == 3:
                ...
                im_neg = (
                    undo_normalize_scale(im_neg)
                    if normalize_and_scale
                    else im_neg.astype(np.uint8)
                )
                axs[2].imshow(im_neg)
                axs[2].axis(axis)
            count += 1
...
def plot_triple_loader(
    ...
    for i, batch in enumerate(data_loader):
        ...
        for j in range(num):
            ...
            im1 = (
                undo_normalize_scale(im1)
                if normalize_and_scale
                else im_src.astype(np.uint8)
            )
            im2 = (
                undo_normalize_scale(im2)
                if normalize_and_scale
                else im_pos.astype(np.uint8)
            )
            im3 = (
                undo_normalize_scale(im3)
                if normalize_and_scale
                else im_pos.astype(np.uint8)
            )
...
def plot_epilines(
    im1, im2, x1s, x2s, F, Npts=50, figsize=(30, 20), unnormalize=True, dpi=350
):
    """
    Args:
        - x1s, x2s: shape (N, 3)
    ...
    for p1, p2, color in zip(x1s[ids, :], x2s[ids, :], colors):
        ...
        a, b, c = l2
        ax2.plot(
            [0, w2], [-c / b, -(c + a * w2) / b], c=color, linestyle="-", linewidth=0.4
        )

        l1 = F.T.dot(p2)
        a, b, c = l1
        ax1.plot(
            [0, w1], [-c / b, -(c + a * w1) / b], c=color, linestyle="-", linewidth=0.4
        )
    plt.gcf().set_dpi(dpi)
    plt.show()
