...
from utils.eval.measure import (
    check_inliers_distr,
    eval_matches_relapose,
    sampson_distance,
)
from utils.eval.model_helper import estimate_matches


def eval_immatch_val_sets(
    net,
    ...
    for scene in scenes:
        ...
        ov_pair_dict = np.load(
            os.path.join(model_dir, "ov_pairs.npy"), allow_pickle=True
        ).item()
        ...
        for i, (im1_name, im2_name) in enumerate(pair_names):
            ...
            count += 1
            try:
                matches, scores, coarse_matches = estimate_matches(
                    net,
                    im1_path,
                    im2_path,
                    ksize=ksize,
            ...
            try:
                # Eval relaposes
                terr, qerr, inls = eval_matches_relapose(
                    matches, im1.K, im2.K, q, t, rthres
                )
                indists = fdists[inls]
                irat = len(inls) / N
    ...
    lprint_(
        f"Pairs {count} match_failed={len(errs.match_failed)} geo_failed={len(errs.geo_failed)} "
    ...
    fdist_ratios, fdist_print = check_inliers_distr(
        errs.fdist, bins=bins, tag="fdist", return_ratios=True
    )
    indist_ratios, indist_print = check_inliers_distr(
        errs.indist, bins=bins, tag="indist", return_ratios=True
    )
    lprint_(cdist_print)
    lprint_(fdist_print)
    ...
    pass_rate = np.array(
        [100.0 * np.mean(np.array(errs.qt) < thre) for thre in range(1, 11, 1)]
    )
    qt_err_mean = np.mean(errs.qt)
    qt_err_med = np.median(errs.qt)
    lprint_(
        "Pose err: qt_mean={:.2f}/{:.2f} qt<[1-10]deg:{}".format(
            qt_err_mean, qt_err_med, pass_rate
        )
    )
