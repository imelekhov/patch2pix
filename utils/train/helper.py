...
def save_ckpt(
    net, epoch, sav_dir, best_vals=None, last_ckpt=False, is_best=False, name=None
):
    ckpt = {
        "last_epoch": epoch,
        "best_vals": best_vals,
...
def get_visdom_plots(prefix="train", env="main", server="localhost", port=9333):
    ...
    """
    plots = Namespace(
        pair_scores=Namespace(pos=None, neg=None),
        cls_ratios=Namespace(
            mpos_gt=None, mpos_pred=None, fpos_gt=None, fpos_pred=None
        ),
        loss=Namespace(
            pair=None, nc=None, cls_mid=None, cls_fine=None, epi_mid=None, epi_fine=None
        ),
        ...
        mem=Namespace(rss=None, vms=None, gpu_maloc=None, gpu_mres=None),
    )
...
def plot_cls_metric(mpred, mgt, thres=0.5, plot=None):
    ...
    try:
        ...
        recall = (
            (TP / Pgt_num).item() if Pgt_num > 0 else (1.0 if Ppred.sum() == 0.0 else 0)
        )  # Correct pred pos among all gt pos
        specifity = (
            (TN / Ngt_num).item() if Ngt_num > 0 else (1.0 if Npred.sum() == 0.0 else 0)
        )  # Correct pred neg among all gt neg
        precision = (
            (TP / Ppred.sum()).item() if Ppred.sum() > 0 else 0
        )  # Correct pos among predicted pos
