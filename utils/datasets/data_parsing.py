...
def parse_3d_points_from_nvm(nvm_file):
    ...
    cam_points = (
        {}
    )  # Map key: index of frame, value: list of indices of 3d points that are visible to this frame.
    ...
    print(
        "Loading finished: camera frames {}, total 3d points {}".format(
            len(cam_points), len(points)
        )
    )
...
class CambridgeIntrinsics:
    ...
    def __init__(self, base_dir, scene, wt=1920, ht=1080, w=1920, h=1080):
        ...
        self.sK = np.array([[wt / w, 0, 0], [0, ht / h, 0], [0, 0, 1]])
        ...
        for im in self.im_list:
            f = self.focals[im]
            K = np.array(
                [[f, 0, self.ox], [0, f, self.oy], [0, 0, 1]], dtype=np.float32
            )
            K = self.sK.dot(K)
...
def get_positive_pairs(cam_points, imlist, thres_min=0.15, thres_max=0.8):
    ...
    for i, im1 in enumerate(imlist):
        for j, im2 in enumerate(imlist):
            ...
            (t, q) = abs2relapose(
                im1.c, im2.c, im1.q, im2.q
            )  # t12 is un-normalized version
            R = quat2mat(q)
            pairs.append(
                Namespace(
                    im1=im1.name,
    ...
    print(
        "Total pairs {} Positive({}<overlap<{}):{}\n".format(
            len(overlaps), thres_min, thres_max, len(pairs)
        )
    )
