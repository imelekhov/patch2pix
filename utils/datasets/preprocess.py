...
def cal_rescale_size(image_size, w, h, k_size=2, scale_factor=1 / 16, no_print=False):
    ...
    wt = int(
        np.floor(w / (max(w, h) / image_size) * scale_factor / k_size)
        / scale_factor
        * k_size
    )
    ht = int(
        np.floor(h / (max(w, h) / image_size) * scale_factor / k_size)
        / scale_factor
        * k_size
    )
    ...
    if not no_print:
        print(
            "Target size {} Original: (w={},h={}), Rescaled: (w={},h={}) , matches resolution: {}".format(
                image_size, w, h, wt, ht, N
            )
        )
    return wt, ht
