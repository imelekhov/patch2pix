...
def init_patch2pix_matcher(args):
    net = load_model(
        args.ckpt, backbone_model_path=args.backbone_model_path, method="patch2pix"
    )
    matcher = lambda imq, imr: estimate_matches(
        net,
        imq,
        imr,
        ksize=args.ksize,
...
def estimate_matches(
    ...
    if eval_type == "coarse":
        with torch.no_grad():
            coarse_matches, scores = net.predict_coarse(
                im1, im2, ksize=ksize, ncn_thres=ncn_thres, mutual=mutual
            )
    ...
    if eval_type == "fine":
        # Fine matches
        with torch.no_grad():
            fine_matches, fine_scores, coarse_matches = net.predict_fine(
                im1, im2, ksize=ksize, ncn_thres=ncn_thres, mutual=mutual
            )
...
def refine_matches(
    ...
    refined_matches, scores, coarse_matches = net.refine_matches(
        im1, im2, coarse_matches, io_thres
    )
