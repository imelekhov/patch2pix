...
get_statis = (
    lambda arr: "Size={} Min={:.2f} Max={:.2f} Mean={:.2f} Median={:.2f}".format(
...
def symmetric_epipolar_distance(pts1, pts2, F, homos=True, sqrt=False):
    ...
    if sqrt:
        ...
        d = np.abs(dd) * (
            1.0 / np.sqrt(l1[0, :] ** 2 + l1[1, :] ** 2)
            + 1.0 / np.sqrt(l2[0, :] ** 2 + l2[1, :] ** 2)
        )
    else:
...
def check_inliers_distr(
    ...
    ratio_print = (
        "{} Sample:{} N(mean/max/min):{:.0f}/{:.0f}/{:.0f}\nRatios(%):".format(
            tag, len(inlier_dists), np.mean(Npts), np.max(Npts), np.min(Npts)
        )
    )
