...
def parse_agrs():
    ...
    # Data loading config
    parser.add_argument("--dataset", type=str, default="PhototourismHmgrphy")
    ...
    parser.add_argument(
        "--lr_decay",
        "-lrd",
        metavar="%s[type] %f[*factor] %d[*step]",
        nargs="*",
        default=None,
    )  # Opt: 'step' 'multistep'
...
def train_epoch(epoch, net, train_loader, train_vis, args, lprint_):
    ...
    skipped = 0
    lprint_(
        f"ksize={ksize} cthres={cthres} cls_dthres={cls_dthres} "
    ...
    for i, batch in enumerate(train_loader):
        ...
        print(im_src.shape, im_pos.shape, Fs.shape)

        # Estimate patch-level matches
        corr4d, delta4d, feats1, feats2 = net.forward(
            im_src, im_pos, ksize=ksize, return_feats=True
        )
        coarse_matches, match_scores = net.cal_coarse_matches(
            corr4d, delta4d, ksize=ksize, upsample=net.upsample, center=True
        )

        if net.panc > 1 and args.ptmax > 0:
            coarse_matches, match_scores = filter_coarse(
                coarse_matches, match_scores, 0.0, True, ptmax=args.ptmax
            )
        # Coarse matches to locate anchors
        coarse_matches = net.shift_to_anchors(coarse_matches)

        # Mid level matching on positive pairs
        mid_matches, mid_probs = net.forward_fine_match(
            feats1,
            feats2,
            coarse_matches,
            psize=net.psize[0],
        ...
        fine_matches, fine_probs = net.forward_fine_match(
            feats1,
            feats2,
            mid_matches,
            psize=net.psize[1],
        ...
        for F, cmat, mmat, fmat, mcls_pred, fcls_pred in zip(
            ...
            mcls_lss = nn.functional.binary_cross_entropy(
                mcls_pred, mcls_pos, reduction="none"
            )
            mcls_lss = (mcls_weights * mcls_lss).mean()

            fcls_weights = fcls_neg.sum() / fcls_pos.sum() * fcls_pos + fcls_neg
            fcls_lss = nn.functional.binary_cross_entropy(
                fcls_pred, fcls_pos, reduction="none"
            )
            ...
            # Epipolar loss
            mids_gt = (
                torch.where(cdist < epi_dthres[0], ones, zeros)
                .nonzero(as_tuple=False)
                .flatten()
            )
            fids_gt = (
                torch.where(mdist < epi_dthres[1], ones, zeros)
                .nonzero(as_tuple=False)
                .flatten()
            )
            ...
            epi_mid = (
                mdist[mids_gt].mean() if len(mids_gt) > 0 else torch.tensor(0).to(mdist)
            )
            epi_fine = (
                fdist[fids_gt].mean() if len(fids_gt) > 0 else torch.tensor(0).to(fdist)
            )
        ...
        # Total loss
        cls_loss = (
            torch.stack(cls_batch_lss).mean()
            if len(cls_batch_lss) > 0
            else torch.tensor(0.0, requires_grad=True).to(net.device)
        )
        epi_loss = (
            torch.stack(epi_batch_lss).mean()
            if len(epi_batch_lss) > 0
            else torch.tensor(0.0, requires_grad=True).to(net.device)
        )
        ...
        if i % plot_step == 0 and i > 0:
            ...
            lprint_(
                "Batch:{} Loss:{}".format(
                    i, train_vis.get_plot_print(train_vis.plots.loss)
                )
            )
            lprint_(
                "Cls_mid:{}".format(train_vis.get_plot_print(train_vis.plots.cls_mid))
            )
            lprint_(
                "Cls_fine:{}".format(train_vis.get_plot_print(train_vis.plots.cls_fine))
            )
            lprint_(
                "Match:{}\n".format(
                    train_vis.get_plot_print(train_vis.plots.match_dist)
                )
            )
        """
    ...
    lprint_(
        ">Epoch:{} Skipped:{} Loss:{}".format(
            epoch + 1, skipped, train_vis.get_plot_print(train_vis.plots.loss)
        )
    )
...
def main():
    ...
    pair_type = (
        match_npy_name.replace("megadepth_pairs.", "")
        .replace("_imrat1.5", "")
        .replace(".npy", "")
    )
    data_tag = "Mega." + pair_type
    # train_set = ImMatchDatasetMega(args.data_root, match_npy, wt=480, ht=320)
    # img_path, crop_size=256, split_type="train", transforms=None

    train_set = PhototourismHmgrphyDataset(
        img_path=Path("/data2/datasets/phototourism"),
        crop_size=256,
        split_type="train",
        transforms=get_train_data_transforms(),
    )
    ...
    thres_tag = "ep{}-{}cls{}-{}".format(
        args.epi_dthres[0], args.epi_dthres[1], args.cls_dthres[0], args.cls_dthres[1]
    )
    train_tag = "_wcls{}wepi{}-{}.lr{}".format(
        args.weight_cls, args.weight_epi[0], args.weight_epi[1], args.lr_init
    )
    ...
    if args.lr_decay:
        ...
        if decay_type == "step":
            train_tag = "{}lrst{}-{}".format(
                train_tag, args.lr_decay[1], args.lr_decay[2]
            )
        elif decay_type == "multistep":
            train_tag = "{}lrms{}-{}".format(
                train_tag, args.lr_decay[1], args.lr_decay[2]
            )
    ...
    # Regressor
    regress_tag = "{}{}_conv{}dim{}str{}fc{}_psz{}-{}a{}".format(
        args.feat_comb,
        args.ptmax,
    ...
    for epoch in range(config.start_epoch, args.epochs):
        ...
        t1 = time.time()
        train_epoch(
            epoch, net, train_loader, args=args, lprint_=lprint_, train_vis=None
        )
        ...
        try:
            ...
            rate = (
                0.34 * pass_rate[0] + 0.33 * pass_rate[4] + 0.33 * pass_rate[9]
            )  # % < 1/5/10 px
            if qt_err < best_vals[2] or rate > best_vals[3]:
                ...
                if rate > best_vals[3]:
                    best_vals[3] = rate
                save_ckpt(
                    net, epoch, out_dir, best_vals=best_vals, name="immatch_best_ckpt"
                )
                lprint_(
                    ">>Save best immatch model: epoch={} qt={:.3f} rate={:.2f}%".format(
                        epoch + 1, qt_err, rate
                    )
                )
