...
from networks.utils import filter_coarse, select_local_patch_feats
...
class Patch2Pix(nn.Module):
    def __init__(self, config):
        ...
        print(
            f"Initialize Patch2Pix: backbone={self.backbone} "
        ...
        if not self.regressor_config:
            ...
            self.regress_mid = FeatRegressNet(
                self.regressor_config, psize=self.psize[0]
            )
            if self.shared:
                self.regress_fine = self.regress_mid
                self.psize[1] = self.psize[0]
            else:
                self.regress_fine = FeatRegressNet(
                    self.regressor_config, psize=self.psize[1]
                )
        ...
        if config.training:
            ...
            print(
                "Freezing feature extractor params upto layer {}".format(
                    self.freeze_feat
                )
            )
    ...
    def forward_fine_match_mini_batch(
        self, feats1, feats2, ibatch, imatches, psize, ptype, regressor
    ):
        ...
        f1s, f2s, _, _ = select_local_patch_feats(
            feats1,
            feats2,
            ibatch,
            imatches,
        ...
        fmatches, io_probs = self.parse_regressor_out(
            out, psize, ptype, imatches, max_val
        )
        return fmatches, io_probs

    def forward_fine_match(
        self, feats1, feats2, coarse_matches, psize, ptype, regressor
    ):
        ...
        for ibatch, imatches in enumerate(coarse_matches):
            ...
            if N > batch_size:
                ...
                for bi, (ist, ied) in enumerate(zip(batch_inds[0:-1], batch_inds[1::])):
                    mini_results = self.forward_fine_match_mini_batch(
                        feats1,
                        feats2,
                        ibatch,
                        imatches[ist:ied],
                ...
                fmatches, io_probs = self.forward_fine_match_mini_batch(
                    feats1, feats2, ibatch, imatches, psize, ptype, regressor
                )
    ...
    def predict_coarse(
        self, im1, im2, ksize=2, ncn_thres=0.0, mutual=False, center=True
    ):
        corr4d, delta4d = self.forward(im1, im2, ksize)
        coarse_matches, match_scores = self.cal_coarse_matches(
            corr4d, delta4d, ksize=ksize, upsample=self.upsample, center=center
        )

        # Filter coarse matches
        coarse_matches, match_scores = filter_coarse(
            coarse_matches, match_scores, ncn_thres, mutual
        )
        return coarse_matches, match_scores

    def predict_fine(
        self, im1, im2, ksize=2, ncn_thres=0.0, mutual=True, return_all=False
    ):
        corr4d, delta4d, feats1, feats2 = self.forward(
            im1, im2, ksize=ksize, return_feats=True
        )
        coarse_matches, match_scores = self.cal_coarse_matches(
            corr4d, delta4d, ksize=ksize, upsample=self.upsample, center=True
        )
        # Filter coarse matches
        coarse_matches, match_scores = filter_coarse(
            coarse_matches, match_scores, ncn_thres, mutual
        ...
        # Mid level matching
        mid_matches, mid_scores = self.forward_fine_match(
            feats1,
            feats2,
            coarse_matches,
            psize=self.psize[0],
        ...
        # Fine level matching
        fine_matches, fine_scores = self.forward_fine_match(
            feats1,
            feats2,
            mid_matches,
            psize=self.psize[1],
    ...
    def refine_matches(self, im1, im2, coarse_matches, io_thres):
        ...
        if type(coarse_matches) == np.ndarray:
            coarse_matches_ = (
                torch.from_numpy(coarse_matches).to(self.device).unsqueeze(0)
            )  # 1, N, 4
    ...
    def cal_coarse_score(self, corr4d, normalize="softmax"):
        ...
        scores_AB = torch.cat(
            [scores_A.view(-1, h1 * w1), scores_B.view(-1, h2 * w2)], dim=1
        )
        score = scores_AB.mean()
        return score

    def cal_coarse_matches(
        self,
        corr4d,
        delta4d,
        ksize=1,
        ...
        (xA_, yA_, xB_, yB_, score_) = corr_to_matches(
            corr4d, delta4d=delta4d, do_softmax=do_softmax, ksize=ksize
        )
        (xA2_, yA2_, xB2_, yB2_, score2_) = corr_to_matches(
            corr4d,
            delta4d=delta4d,
        ...
        matches_ = upsample * torch.cat([xA_, yA_, xB_, yB_], dim=1).permute(
            0, 2, 1
        )  # B, N, 4
        if center:
            delta = upsample // 2
            matches_ += (
                torch.tensor([[delta, delta, delta, delta]]).unsqueeze(0).to(matches_)
            )
