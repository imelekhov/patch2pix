...
def select_local_patch_feats(
    ...
    # Ids for local patch
    get_x_pids = (
        lambda x, w, ds: ((x.view(-1, 1) + dx).view(-1) // ds)
        .long()
        .clamp(min=0, max=w // ds - 1)
    )
    get_y_pids = (
        lambda y, h, ds: ((y.view(-1, 1) + dy).view(-1) // ds)
        .long()
        .clamp(min=0, max=h // ds - 1)
    )
...
def filter_coarse(coarse_matches, match_scores, ncn_thres=0.0, mutual=True, ptmax=None):
    ...
    for imatches, iscores in zip(coarse_matches, match_scores):
        _, ids, counts = np.unique(
            imatches.cpu().data.numpy(), axis=0, return_index=True, return_counts=True
        )
        if mutual:
...
def sym_epi_dist(matches, F, sqrt=True, eps=1e-8):
    ...
    if sqrt:
        d = dd.abs() * (
            1.0 / (eps + l1[0, :] ** 2 + l1[1, :] ** 2).sqrt()
            + 1.0 / (eps + l2[0, :] ** 2 + l2[1, :] ** 2).sqrt()
        )
    else:
        d = dd ** 2 * (
            1.0 / (eps + l1[0, :] ** 2 + l1[1, :] ** 2)
            + 1.0 / (eps + l2[0, :] ** 2 + l2[1, :] ** 2)
        )
    return d.float()
