...
def corr_to_matches(
    ...
    if scale == "centered":
        XA, YA = np.meshgrid(
            np.linspace(-1, 1, fs2 * ksize), np.linspace(-1, 1, fs1 * ksize)
        )
        XB, YB = np.meshgrid(
            np.linspace(-1, 1, fs4 * ksize), np.linspace(-1, 1, fs3 * ksize)
        )
    elif scale == "positive":
        # Upsampled resolution linear space
        XA, YA = np.meshgrid(
            np.linspace(0, 1, fs2 * ksize), np.linspace(0, 1, fs1 * ksize)
        )
        XB, YB = np.meshgrid(
            np.linspace(0, 1, fs4 * ksize), np.linspace(0, 1, fs3 * ksize)
        )
    ...
    if invert_matching_direction:
        ...
        iB = (
            IB.view(-1)[idx_A_Bvec.view(-1)].view(batch_size, -1).contiguous()
        )  # b, h1*w1
        ...
        iA = (
            IA.view(-1)[idx_B_Avec.view(-1)].view(batch_size, -1).contiguous()
        )  # b, h2*w2
...
def corr_to_matches_topk(
    ...
    JA, IA = np.meshgrid(range(fs2), range(fs1))
    JB, IB = np.meshgrid(range(fs4), range(fs3))
    JA, IA = torch.LongTensor(JA).view(1, -1).to(device), torch.LongTensor(IA).view(
        1, -1
    ).to(device)
    JB, IB = torch.LongTensor(JB).view(1, -1).to(device), torch.LongTensor(IB).view(
        1, -1
    ).to(device)
    ...
    if invert_matching_direction:
        ...
        # Max and argmax
        match_A_vals, idx_A_Bvec = torch.topk(
            nc_A_Bvec, topk, dim=3, largest=True, sorted=True
        )
        score = match_A_vals.view(batch_size, -1)
        ...
        match_B_vals, idx_B_Avec = torch.topk(
            nc_B_Avec, topk, dim=1, largest=True, sorted=True
        )
        score = match_B_vals.view(batch_size, -1)
