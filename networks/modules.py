...
L2Normalize = lambda feat, dim: feat / torch.pow(
    torch.sum(torch.pow(feat, 2), dim=dim) + 1e-6, 0.5
).unsqueeze(dim)
...
def maxpool4d(corr4d_hres, k_size=4):
        ...
        for j in range(k_size):
            for k in range(k_size):
                for l in range(k_size):
                    sl = corr4d_hres[
                        :, :, i::k_size, j::k_size, k::k_size, l::k_size
                    ]  # Support batches
                    slices.append(sl)
    ...
    if torch.__version__ >= "1.6.0":
        ...
        max_j = torch.fmod(
            max_idx.sub(max_l).floor_divide(k_size).sub(max_k).floor_divide(k_size),
            k_size,
        )
        max_i = (
            max_idx.sub(max_l)
        ...
        max_j = torch.fmod(
            max_idx.sub(max_l).div(k_size).sub(max_k).div(k_size), k_size
        )
        max_i = (
            max_idx.sub(max_l).div(k_size).sub(max_k).div(k_size).sub(max_j).div(k_size)
        )
...
class FeatCorrelation(torch.nn.Module):
    ...
    def forward(self, feat1, feat2):
        ...
        if self.shape == "3D":
            correlation = correlation.view(b, h1, w1, h2 * w2).permute(
                0, 3, 1, 2
            )  # [b, h2*w2, h1, w1]
        elif self.shape == "4D":
            correlation = correlation.view(b, h1, w1, h2, w2).unsqueeze(
                1
            )  # [b, 1, h1, w1, h2, w2]
        return correlation
...
class FeatRegressNet(nn.Module):
    def __init__(self, config, psize=16, out_dim=5):
        ...
        self.psize = psize
        self.conv_strs = (
            config.conv_strs if "conv_strs" in config else [2] * len(config.conv_kers)
        )
        self.conv_dims = config.conv_dims
        self.conv_kers = config.conv_kers
        self.feat_comb = (
            config.feat_comb
        )  # Combine 2 feature maps before the conv or after the conv
        self.feat_dim = (
            config.feat_dim if self.feat_comb == "post" else 2 * config.feat_dim
        )
        self.fc_in_dim = (
            config.conv_dims[-1] * 2
            if self.feat_comb == "post"
            else config.conv_dims[-1]
        )
        ...
        print(
            f"FeatRegressNet:  feat_comb:{self.feat_comb} "
    ...
    def make_conv_layers(self, in_dim, conv_dims, conv_kers, bias=False):
        ...
        for out_dim, kernel_size, stride in zip(conv_dims, conv_kers, self.conv_strs):
            layers.append(
                nn.Conv2d(
                    in_dim, out_dim, kernel_size, stride=stride, padding=1, bias=bias
                )
            )
...
def init_optimizer(params, config):
    if config.opt == "adam":
        optimizer = torch.optim.Adam(
            params, lr=config.lr_init, weight_decay=config.weight_decay
        )
        print(
            "Setup  Adam optimizer(lr={},wd={})".format(
                config.lr_init, config.weight_decay
            )
        )

    elif config.opt == "sgd":
        optimizer = torch.optim.SGD(
            params, momentum=0.9, lr=config.lr_init, weight_decay=config.weight_decay
        )
        print(
            "Setup  SGD optimizer(lr={},wd={},mom=0.9)".format(
                config.lr_init, config.weight_decay
            )
        )
    ...
    if "lr_decay" in config and config.lr_decay:
        if config.lr_decay[0] == "step":
            decay_factor, decay_step = float(config.lr_decay[1]), int(
                config.lr_decay[2]
            )
            ...
            print(
                f"Setup StepLR Decay: decay_factor={decay_factor} "
            ...
            print(
                f"Setup MultiStepLR Decay: decay_factor={decay_factor} "
